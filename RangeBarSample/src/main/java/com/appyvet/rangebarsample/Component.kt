package com.appyvet.rangebarsample

/**
 * Enumeration of the components to be changed in colorChanged. Only needed for
 * the RangeBar Sample Activity, not for the library project RangeBar itself.
 *
 * @author deric
 */
enum class Component {
    BAR_COLOR, CONNECTING_LINE_COLOR, PIN_COLOR, THUMB_COLOR_PRESSED, THUMB_COLOR, THUMB_BOUNDARY_COLOR, TICK_COLOR, TEXT_COLOR, TICK_LABEL_COLOR, TICK_LABEL_SELECTED_COLOR
}